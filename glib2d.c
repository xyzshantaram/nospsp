...
typedef enum { RECTS, LINES, QUADS, POINTS } Obj_Type;
...
g2dTexture g2d_draw_buffer = {512,
                              512,
                              G2D_SCR_W,
                              G2D_SCR_H,
...
g2dTexture g2d_disp_buffer = {
    512,   512,          G2D_SCR_W, G2D_SCR_H, (float)G2D_SCR_W / G2D_SCR_H,
    false, (g2dColor *)0};
...
void g2dBeginRects(g2dTexture *tex) { _g2dBeginCommon(RECTS, tex); }
...
void g2dBeginQuads(g2dTexture *tex) { _g2dBeginCommon(QUADS, tex); }

void g2dBeginPoints() { _g2dBeginCommon(POINTS, NULL); }
...
void _g2dEndRects() {
    ...
    int v_size = v_tex_size * sizeof(short) + v_color_size * sizeof(g2dColor) +
    ...
    for (i = 0; i < rctx.n; i += 1) {
        ...
        } else // Several sprites per object for a better texture cache use
        {
            ...
            for (u = 0.f; u < 1.f; u += step) {
                ...
                vi = _g2dSetVertex(vi, i, (u + step > 1.f ? 1.f : u + step),
                                   1.f);
            }
        }
    }
...
void g2dEnd() {
    ...
    if (rctx.tex == NULL)
        ...
        sceGuTexImage(0, rctx.tex->tw, rctx.tex->th, rctx.tex->tw,
                      rctx.tex->data);
    }
...
void g2dPush() {
    if (tstack_size >= TSTACK_MAX) return;
...
void g2dSetCoordInteger(bool use) { rctx.use_int = use; }

/* Scale functions */

void g2dResetGlobalScale() { global_scale = 1.f; }
...
void g2dSetGlobalScale(float scale) { global_scale = scale; }
...
void g2dResetColor() { rctx.cur_obj.color = DEFAULT_COLOR; }

void g2dResetAlpha() { rctx.cur_obj.alpha = DEFAULT_ALPHA; }

void g2dGetAlpha(g2dAlpha *alpha) {
    if (alpha != NULL) *alpha = rctx.cur_obj.alpha;
}

void g2dSetColor(g2dColor color) {
    rctx.cur_obj.color = color;

    if (++rctx.color_count > 1) rctx.use_vert_color = true;
}
...
void g2dSetAlphaRelative(int alpha) { g2dSetAlpha(rctx.cur_obj.alpha + alpha); }
...
void g2dGetRotationRad(float *radians) {
    if (radians != NULL) *radians = rctx.cur_obj.rot;
}

void g2dGetRotation(float *degrees) {
    if (degrees != NULL) *degrees = rctx.cur_obj.rot * M_180_PI;
}

void g2dSetRotationRad(float radians) {
    if (radians == rctx.cur_obj.rot) return;
    ...
    if (radians != 0.f) rctx.use_rot = true;
}

void g2dSetRotation(float degrees) { g2dSetRotationRad(degrees * M_PI_180); }
...
void _swizzle(unsigned char *dest, unsigned char *source, int width,
              int height) {
    int i, j;
...
g2dTexture *_g2dTexLoadPNG(FILE *fp) {
    ...
    for (y = 0; y < height; y++) {
        ...
        for (x = 0; x < width; x++) tex->data[x + y * tex->tw] = line[x];
    }
